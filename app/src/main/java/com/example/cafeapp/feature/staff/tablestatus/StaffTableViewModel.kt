...
class StaffTableViewModel(
    ...
    internal var repository: TableRepository =
        TableRepository(RetrofitClient.api)

    private val _tables =
        MutableStateFlow<Resource<List<TableResponse>>>(
            Resource.Idle()
        )

    val tables: StateFlow<Resource<List<TableResponse>>> =
        _tables
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (
                    ...
                    _tables.value =
                        Resource.Error("Failed to fetch tables")
                }
